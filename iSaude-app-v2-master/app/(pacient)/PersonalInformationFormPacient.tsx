...
import { useRegister } from "../../lib/context/RegisterContext"; // Importa o hook do contexto

export default function PersonalInformationFormPacient(props: any) {
  const { formData, updateFormData } = useRegister(); // Usa o contexto
  ...
  const [verified, setVerified] = useState(false); // A lógica de verificação por SMS pode ser aprimorada
  ...
  const handleContinue = () => {
    ...
    // Salva os dados no contexto antes de navegar
    updateFormData({ nome, cpf: cpf.replace(/\D/g, ""), email, telefone });

    if (!verified) { // Simulação de verificação de telefone
      setShowModal(true);
      return;
    }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#fff", paddingTop: insets.top }}>
      ...
      <SucessVerificationModal
        ...
        onConfirm={() => navigation.navigate("BasicInformationFormPacient")}
      />
    </View>
  );
}