...
import { useRegister } from "../../lib/context/RegisterContext";
import { createUser } from "../../lib/auth"; // Importa a função da API

export default function PasswordInformationFormPacient(props: any) {
  const { formData, updateFormData, resetFormData } = useRegister();
  const [senha, setSenha] = useState(formData.password || "");
  const [confirmar, setConfirmar] = useState(formData.password || "");
  ...
  const handleRegister = async () => {
    if (!isValid) {
        if (!senha || !confirmar) setErro("Preencha todos os campos.");
        else if (senha !== confirmar) setErro("As senhas não coincidem.");
        else setErro("A senha precisa ser forte e atender todos os requisitos.");
        return;
    }
    ...
    const finalFormData = { ...formData, password: senha };
    
    try {
      // Chama a função da API com os dados completos do formulário
      ...
      const errorMsg = error.response?.data?.message || "Ocorreu um erro ao criar a conta. Tente novamente.";
      setErro(errorMsg);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#fff", paddingTop: insets.top }}>
      ...
      <View style={{ 
        ...
        <Button onPress={handleRegister} icon={loading ? <ActivityIndicator color="white" /> : <ArrowRight size={18} color="white" />} disabled={!isValid || loading}>
