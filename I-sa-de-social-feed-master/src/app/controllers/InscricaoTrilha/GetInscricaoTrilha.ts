...
async function getInscricaoTrilha(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(filterObject).length > 0) {
      results = await inscricaoTrilhaService.getInscricoesTrilha(filterObject);
    } else {
      results = await inscricaoTrilhaService.getInscricoesTrilha({});
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: "Inscrição Trilha not found" });
    }
...
export default getInscricaoTrilha;
