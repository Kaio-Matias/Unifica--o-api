...
async function getTrilha(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(filterObject).length > 0) {
      results = await trilhaService.getTrilhas(filterObject);
    } else {
      results = await trilhaService.getTrilhas({});
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: "Trilha not found" });
    }
