...
async function getProgressoAula(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(filterObject).length > 0) {
      results = await progressoAulaService.getProgressosAula(filterObject);
    } else {
      results = await progressoAulaService.getProgressosAula({});
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: "Progresso Aula not found" });
    }
...
export default getProgressoAula;
