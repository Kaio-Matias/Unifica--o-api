...
async function getRespostaUsuario(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(filterObject).length > 0) {
      results = await respostaUsuarioService.getRespostasUsuario(filterObject);
    } else {
      results = await respostaUsuarioService.getRespostasUsuario({});
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: "Resposta Usuario not found" });
    }
...
export default getRespostaUsuario;
