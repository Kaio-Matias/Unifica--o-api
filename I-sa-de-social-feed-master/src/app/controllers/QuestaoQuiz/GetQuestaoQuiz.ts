...
async function getQuestaoQuiz(req: Request, res: Response) {
  try {
    ...
    if (id) filterObject.id_questao = parseInt(id)
    if (order) {
    ...
    if (Object.keys(filterObject).length > 0) {
      results = await questaoQuizService.getQuestoesQuiz(filterObject);
    } else {
      results = await questaoQuizService.getQuestoesQuiz({});
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: "Questao quiz not found" });
    }
