...
async function getComentario(req: Request, res: Response) {
  try {
    ...
    if (req.headers['order']) {
      ...
      const order = typeof orderHeader === "string" ? JSON.parse(orderHeader) : orderHeader;
    ...
    if (Object.keys(filterObject).length > 0) {
      results = await comentarioService.getComentarios(filterObject);
    } else {
      results = await comentarioService.getComentarios({});
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: "Comentário not found" });
    }
