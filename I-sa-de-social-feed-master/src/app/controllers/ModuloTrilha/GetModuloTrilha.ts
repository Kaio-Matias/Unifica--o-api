...
async function getModuloTrilha(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(filterObject).length > 0) {
      results = await moduloTrilhaService.getModulosTrilha(filterObject);
    } else {
      results = await moduloTrilhaService.getModulosTrilha({});
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: "Modulo trilha not found" });
    }
...
export default getModuloTrilha;
