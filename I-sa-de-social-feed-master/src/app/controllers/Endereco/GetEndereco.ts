...
async function getEndereco(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(filterObject).length > 0) {
      results = await enderecoService.getEnderecos(filterObject);
    } else {
      results = await enderecoService.getEnderecos({});
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: "Endereço not found" });
    }
