...
async function getProfissionalDetalhes(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(filterObject).length > 0) {
      results = await profissionalDetalhesService.getProfissionaisDetalhes(filterObject);
    } else {
      results = await profissionalDetalhesService.getProfissionaisDetalhes({});
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: "Profissional Detalhes not found" });
    }
...
export default getProfissionalDetalhes;
