...
async function getAula(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(filterObject).length > 0) {
      results = await aulaService.getAulas(filterObject);
    } else {
      results = await aulaService.getAulas({});
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: "Aula not found" });
    }
