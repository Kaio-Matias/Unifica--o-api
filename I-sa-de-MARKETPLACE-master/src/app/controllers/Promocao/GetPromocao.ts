...
async function getPromocao(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(objectFilter).length > 0) {
      results = await getService.getPromocao(objectFilter)
    }
    else {
      results = await getService.getPromocao({})
    }
    ...
    if (results?.length === 0) {
      return res.status(404).json({ message: "Promoção not found" });
    }
