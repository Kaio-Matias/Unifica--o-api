...
async function getPharmacyProduct(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(objectFilter).length > 0) {
      results = await getService.getPharmacyProduct(objectFilter)
    }
    else {
      results = await getService.getPharmacyProduct({})
    }
...
export default getPharmacyProduct;
