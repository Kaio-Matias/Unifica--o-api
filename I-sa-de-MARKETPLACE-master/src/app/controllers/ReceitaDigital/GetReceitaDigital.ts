...
async function getReceitaDigital(req: Request, res: Response) {
  try {
    ...
    if (Object.keys(objectFilter).length > 0) {
      results = await getService.getReceitaDigital(objectFilter)
    }
    else {
      results = await getService.getReceitaDigital({})
    }
    ...
    if (results?.length === 0) {
      return res.status(404).json({ message: "Receita Digital not found" });
    }
...
export default getReceitaDigital;
